...
available_outcomes <- read.csv("outcomes.csv")
choices <- as.vector(available_outcomes$label)
available_outcomes$label <- as.character(available_outcomes$label)
available_outcomes$file <- as.character(available_outcomes$file)
...
server <- function(input, output) {   
    ...
    observe({
        ...
        if(length(input$outcome) != 0){
            ...
            selected_file <- available_outcomes$file[available_outcomes$label == selected_outcome]
            
            outcome_data <- reactive({
                read.csv(paste(selected_file,".csv",sep=""))
            })
