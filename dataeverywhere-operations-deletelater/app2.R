...
observe({
  invalidateLater(180000,NULL) # update once an hour  
...
server <- function(input, output) { 
  ...
  observe({
    #browser()
    dat <- sharedValues$data
    if(nrow(dat) > 0){
    
    roundData <- reactive({getRoundData(dsDSRoundA[1,]["ExtID"],sharedValues$data)})
  ...
  output$hhdcgauge <- renderGvis({
    #browser()
    data <- getRoundData(dsDSRoundA[1,]["ExtID"],sharedValues$data)
    getContingencyTable(roundData(),"household",period = "round")
  ...
  output$hhdegauge <- renderGvis({
    # roundData() here is used PURELY to get this widget to "react" to changes in data periodically
    # as it is a reactive function
    roundData()
  ...
  output$inddcgauge <- renderGvis({
    indData <- getIndividualRoundData(getCurrentRound(),sharedValues$data)
  ...
  #############################################################################
  data <- reactive({
    getVAData(sharedValues$data)
  })
  
  
  output$vadcgauge <- renderGvis({
    #browser()
    getContingencyTable(data(),"va","round")
  ...
  observe({
    ...
    if(!is.null(input$yearsva)){
      #browser()
      vadata <- getVAData(sharedValues$data)
