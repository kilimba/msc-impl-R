...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    conditionalPanel(
      condition = "input.tabs != 'dashboard' & input.tabs != 'typeC'",
...
dataSetup <- function(){
  ...
  # check if last update is at least 10 min ago
  else if(abs(as.numeric(difftime(sharedValues$lastupdate,Sys.time()),units = "mins" )) > 10){
    ...
    cat(diff,"minutes since last update...\n")
    #stop("less than 10 minutes have elapsed...",error =NULL)
    cat("less than 10 minutes have elapsed...\n")
    return()
  }
